...
--?—†?—†?—†ADMIN?—†?—†?—†
    --?˜…INSERT MINI DATA
    INSERT INTO ADMIN (aID, aPW, aNAME, aKING) VALUES ('aKING', '111', 'ìµœê³ ê´?ë¦¬ìž?´ë¦?', 1);
    INSERT INTO ADMIN (aID, aPW, aNAME) VALUES ('aaa', '111', 'ë°•ë°•ë°?');
    SELECT * FROM ADMIN;
    --(1)LOGIN CHECK
...
--?—†?—†?—†MEMBER?—†?—†?—†
    --?˜…INSERT MINI DATA
    INSERT INTO MEMBER (mID, mPW, mNAME, mBIRTH, mEMAIL) VALUES ('aaa', '111', 'ë°•ë°•ë°?', '99-09-09', 'park@park.com');
    SELECT * FROM MEMBER;
    --(1)LOGIN CHECK
...
--?—†?—†?—†QNABOARD?—†?—†?—†
    ...
    --(1)LIST(startROW~endROW)
    SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT qID, NVL(mID, AID) ID, qTITLE, qCONTENT, qFILENAME, qGROUP, qSTEP, qINDENT, qIP FROM QNABOARD ORDER BY qGROUP DESC, qSTEP) A)
...
--?—†?—†?—†myBAGBOARD?—†?—†?—†
    ...
    INSERT INTO myBAGBOARD (bID, mID, bNAME, bCONTENT, bFILENAME, bIP) 
        VALUES (myBAG_SEQ.NEXTVAL, 'aaa', 'THISISMYBAG', 'mybag', null, '112.169.33.198');
    ...
    SELECT * FROM myBAGBOARD WHERE bID='1';
    --(6)MODIFY myBAGBOARD
...
--?—†?—†?—†REPLYmyBAG?—†?—†?—† 
    ...
    SELECT * FROM REPLYmyBAG WHERE rID='3';
    --(5)MODIFY REPLYmyBAG
...
--?—†?—†?—†LIKEmyBAG?—†?—†?—†
    ...
    SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT B.* FROM myBAGBOARD B, LIKEmyBAG L 
                                                    WHERE B.MID=L.MID AND B.BID= L.BID ORDER BY lRDATE DESC) A)
        WHERE RN BETWEEN 1 AND 3;
